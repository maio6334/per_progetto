...
def visually_compare2(logfile:str,columns:list):
    ...
    df=df.groupby(['rate','coding'],as_index=False)[["diff", "avg_te", "avg_td"]].mean()
...
def visually_compare(logfile:str,columns:list):
    ...
    l=[]
    for cod in coding:
        for dim in columns[3:]: # from diff  
            filt=df[df['coding']==cod][dim].groupby(level=0).mean()
            ser_name=cod + '-' + dim
            filt.name=ser_name
            l.append(filt)
    # plot time VS BER
    dft=pd.concat(l[1:],axis=1)
    dft.plot(ax=axs[0])
    #plot error vs BER
    dfe=pd.concat(l,axis=1)
    dfe.plot(l[0],ax=axs[1])    
    plt.show()
    pass
